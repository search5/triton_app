...
def board_list():
    ...
    if keyword:
        query = query.where(Board.subject.like(f'%{keyword}%'))
    query = query.order_by(db.desc(Board.seq))

    page = db.paginate(query, per_page=10)

    return render_template("board_list.jinja2",
                           pagination=page, keyword=keyword)


@board_app.route("/<int:number>")
def board_view(number):
    article_record = db.one_or_404(db.select(Board).where(Board.seq == number))

    article_record.hit += 1
    db.session.commit()

    return render_template("board_view.jinja2",
...
def board_write_save():
    ...
    if not current_user.is_anonymous:
        valid_schema = filter(lambda x: x[0] not in ('writer_name', 'password'),
                              valid_schema.items())
    ...
    return jsonify(success=success_result,
                   message=v.errors,
                   article_id=article.seq)


@board_app.route('/<int:number>/modify')
def board_modify_view(number):
    db_record = db.session.execute(
        db.select(Board).where(Board.seq == number)).scalar_one()
...
def board_modify_save(number):
    ...
    # 로그인한 사용자는 작성자명과 비밀번호의 유효성을 검증하지 않도록 합니다.
    if not current_user.is_anonymous:
        valid_schema = filter(lambda x: x[0] != 'password',
                              valid_schema.items())
    ...
    if success_result:
        ...
        article = db.session.execute(
            db.select(Board).where(Board.seq == number)).scalar_one()
        if ((article.password == request.form.get('password'))
                or (article.user_id == current_user.get_id())):
            article.subject = request.form.get('subject')
            article.content = request.form.get('content')
            db.session.add(article)
            db.session.commit()
...
def board_login_proc():
    ...
    if success_result:
        ...
        user = db.session.execute(
            db.select(Member).where(Member.id == login_id)).scalar_one()
        if not user:
...
def comment_list(number):
    board_comments = db.session.execute(
        db.select(BoardComments).where(BoardComments.board_seq == number)
    ).scalars()
...
def comment_write(number):
    ...
    return (redirect(url_for('triton_board.board_view', number=number)),
            {'x-comment_id': seq})


@board_app.route("/<int:number>/comments/<int:comment_seq>",
                 methods=["PUT"])
