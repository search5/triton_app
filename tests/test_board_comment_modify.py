...
def test_guest_comment_modify_input(client, write_article):
...
def test_logined_comment_modify_input(client_user, write_comment):
...
def test_guest_comment_modify(client, write_comment):
    """손님 상태에서 댓글 편집 후 저장 기능 테스트"""
    article_id = write_comment.get("article_id")
    comment_id = write_comment.get("comment_id")

    req_http = client.put(f'/board/{article_id}/comments/{comment_id}', json={
...
def test_logined_comment_modify(client_user, write_comment_loggined):
